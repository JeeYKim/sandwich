	...
	$("#insert").on("click", function(e){ //작성하기 버튼
		e.preventDefault();
		 if($('#STAFF_ID').val()==""){
			...
			return false;
		 } else if($('#STAFF_COST').val()==""){
			 alert("급여를 입력해 주세요")
		 
		 } else if($('#COST_DATE').val()==""){
			 alert("지급 일자를 입력해 주세요.");
				return false;
		 }else{
		fn_insertCost();
		}
...
function fn_insertCost(){
	...
	comSubmit.setUrl('/sandwich/staffMoneyInsert.jy');
	comSubmit.submit();
}
								...
								<c:forEach var="staffMoney"  items="${staffMoney}" varStatus="stat">
									...
									<tr class="gradeA even" role="row">
										...
										<td style="text-align:center;vertical-align:middle;">
										 ...
										 <a href="${StaffMoneyDelete}"><input type="image" src="resources/images/delete.png" onclick="return delchk()"></a></td>									
									 </tr>
								</c:forEach>
								<!--  등록된 상품이 없을때 -->
									<c:if test="${staffMoney}==0">
